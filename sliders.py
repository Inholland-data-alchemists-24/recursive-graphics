...
def create_sliders(
        parent_frame: Frame, sliders_init_data: Collection[tuple], num_columns: int, x_padding: int, y_padding: int, command
) -> list[Scale]:
    """
    ...
    for slider_args in sliders_init_data:
        ...
        sliders.append(create_slider(parent_frame, x_padding, y_padding, command, *slider_args, *placement))
    return sliders
...
def create_slider(
        parent: Frame, x_padding: int, y_padding: int, command, label: str,
        from_: int | float, to: int | float, resolution: int | float, variable, row: int, column: int
) -> Scale:
    """
    ...
    """
    slider = Scale(parent, label=label, from_=from_, to=to, orient=HORIZONTAL, resolution=resolution, variable=variable, command=command)
